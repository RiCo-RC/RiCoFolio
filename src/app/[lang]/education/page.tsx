...
import { ExternalLink, Info, Briefcase, GraduationCap } from "lucide-react";
...
export default function EducationPage() {
	...
	const base = useMemo(
		() => education.filter((e) => matchesProfile(e.profile, profile)),
		[profile],
	);
	const filtered = useMemo(() => {
		...
		return base.filter((e) =>
			activeTags.some((tag) =>
				e.profile.includes(tag as Education["profile"][number]),
			),
		);
	}, [base, activeTags]);
	...
	return (
		<div>
			<SectionHero
				title={t.pages.education.heroTitle}
				description={t.pages.education.heroDescription}
				badge="Education"
			/>
			...
			<div className="page-container py-12">
				<div className="flex items-center justify-between mb-8 gap-4 flex-wrap">
					<span className="text-sm text-text-secondary font-mono">
						{filtered.length} {t.pages.education.labelFilter}
					</span>
					<AdvancedFilter
								...
								<div className="flex gap-2 pt-2 border-t border-border">
									{item.schoolUrl && (
										<Button
											variant="ghost"
											size="sm"
											onClick={() => window.open(item.schoolUrl, "_blank")}
										>
